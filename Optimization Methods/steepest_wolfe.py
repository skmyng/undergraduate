...
def wolfe(x,d,maxiter=100):
    alphamin = 0.0
    alphamax = 10.0
    while f(x+alphamax*d) <= f(x)+c1*alphamax*np.dot(gradf(x),d):
        alphamax = alphamax*2

    alpha0 = (alphamin+alphamax)/2
    alpha = alpha0

    for i in range(maxiter):
        if f(x+alpha*d) > f(x)+c1*alpha*np.dot(gradf(x),d):
            alphamax = alpha
        elif np.dot(gradf(x+alpha*d),d) >= c2*np.dot(gradf(x),d):
            break
        else:
            alphamin = alpha
